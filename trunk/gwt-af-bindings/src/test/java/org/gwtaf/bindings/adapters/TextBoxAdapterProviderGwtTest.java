...
/**
 * A test of the {@link TextBoxAdapterProvider}.
 * 
 * @author Sergey Vesselov
 */
public class TextBoxAdapterProviderGwtTest extends GWTTestCase {

	/**
	 * Verifies that data bindings reflect information from UI to model and
	 * back.
	 */
	public void testTextBoxBindings() {
		...
		Binding<TextBox, String, Person, String> binding = builder
				.createBinding(textBox, "text", String.class, serge,
						"fullname", String.class, new TextBoxAdapterProvider());

		// assert the binding is bound.
		Assert.assertTrue(binding.isBound());

		// change the UI values
		textBox.setText("serge");

		// fire off events
		ValueChangeEvent.fire(textBox, "serge");
		...
		Assert.assertEquals("coffee consumer", textBox.getText());
	}
